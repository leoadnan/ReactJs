...
class AddTicketForm extends Component {
   ...
   // handleClose() { this.refs.alertBox.close(); }
   render() {
      var alertBox = <BootstrapAlert className="alert-warning fade in" ref="alertBox"/>;
      var formHtml = <form onSubmit={this.handleSubmitEvent.bind(this)}>
         ...
         <div className="form-group">
            ...
            (<span id="maxlength">{this.state.char_Left}</span> characters left)
            <textarea
               ref="comment"
               className="form-control"
               rows="3"
